...
public class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        // HandleCreated fires on first handle creation regardless of visibility,
        // so the timer starts even when StartMinimized suppresses the Load event.
        HandleCreated += (_, _) =>
        {
            ApplySettingsToUi();
            _timer.Start();
    ...
    {
        var iconPath = Path.Combine(AppContext.BaseDirectory, "pchub.ico");
    ...
    private void BuildSettingsTab()
    {
        ...
        _settingsTab.Controls.Add(_startMinimizedCheck);
        y += 36;
    ...
    private async void OnTimerTick(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            // Run on thread pool — LibreHardwareMonitor's hardware.Update() makes
            // driver/WMI calls that can block for hundreds of ms at 100% CPU load.
    ...
    {
        // Suppress initial window show when StartMinimized is enabled — no flash at all
        if (!IsHandleCreated && _settings.StartMinimized)
        {
            CreateHandle();
            value = false;
        }
