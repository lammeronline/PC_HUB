...
static const unsigned long WEATHER_RETRY_INTERVAL_MS  = WEATHER_RETRY_INTERVAL_SEC  * 1000UL;
...
void loop() {
    ...
    if (now - lastSensorUpdate >= SENSOR_INTERVAL_MS) {
        updateSensors(currentData);
        updateLED(currentData.temperature, currentData.humidity, currentData.iaq, currentData.bme_ok);
    ...
    {
        unsigned long weatherInterval = weatherData.ok ? WEATHER_UPDATE_INTERVAL_MS : WEATHER_RETRY_INTERVAL_MS;
        if (now - lastWeatherUpdate >= weatherInterval && !weatherTaskBusy()) {
            triggerWeatherFetch();
