...
private:
    ...
    void drawStatusIcon(int x, int y, uint16_t col, uint8_t kind) {
        ...
        switch (kind) {
            ...
            case 4: { // PC monitor — screen bezel + stand + base
                tft->drawRect(x+1, y+1, 12, 8, col);
                tft->fillRect(x+2, y+2, 10, 6, C_HEADER);
                tft->fillRect(x+6, y+9, 2, 3, col);
                tft->drawLine(x+3, y+12, x+10, y+12, col);
                break;
            }
        }
    }
    ...
    void drawWeatherIcon(int cx, int cy, int code, bool isDay, int R = 9, int dR = -1) {
        ...
        {
            int bw = max(2, dR/3), bh = max(4, dR*3/4);
            int ty = py, my = py + bh/2, ey = py + bh;
            // Classic Z-shaped bolt with proper zigzag form
            int mw = bw / 2;
            tft->fillTriangle(cx - bw,      ty, cx + bw, my - bh/4, cx - mw, my,     C_AMBER);
            tft->fillTriangle(cx + bw,      my - bh/4, cx + mw, my, cx - mw, my,     C_AMBER);
            tft->fillTriangle(cx - mw,      my, cx + mw, my, cx + mw/2, my + bh/4, C_AMBER);
            tft->fillTriangle(cx - mw,      my, cx + mw/2, my + bh/4, cx - bw, ey,   C_AMBER);
        }
    }
    ...
    void drawForecastTab(const WeatherData &weather) {
        ...
        // Title strip: clear only when draw() didn't pre-clear (forecastDirty-only update)
        if (!contentDirty) tft->fillRect(0, CY, SW, 28, C_BG);
        char title[48];
        String city = RuntimeSettings::weatherCity();
        ...
        if (!weather.ok) {
            if (!contentDirty) tft->fillRect(0, CY + 28, SW, CONTENT_H - 28, C_BG);
        ...
        for (int i = 0; i < 7; i++) {
            int y = ROW_Y + i * ROW_H;
            // Clear row strip only when draw() didn't already clear the content area
            if (!contentDirty) tft->fillRect(0, y, SW, ROW_H, C_BG);
            ...
            drawWeatherIcon(148, y + 10, d.weather_code, true, 4, 5);
        ...
        // Bottom gap (8px) — only on weather-only updates
        if (!contentDirty) tft->fillRect(0, ROW_Y + 7 * ROW_H, SW, (CY + CONTENT_H) - (ROW_Y + 7 * ROW_H), C_BG);
        forecastDirty = false;
    }
